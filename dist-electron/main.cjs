...
function loadPackagedDesktopEnv() {
  const desktopEnvPath = (0, import_path.join)(process.resourcesPath, "desktop.env");
  if ((0, import_fs.existsSync)(desktopEnvPath)) {
    (0, import_dotenv.config)({ path: desktopEnvPath });
  }
}
...
import_electron.app.whenReady().then(async () => {
  ...
  const appendLog = (chunk) => {
    serverLog = (serverLog + chunk.toString()).slice(-4e3);
  };
  ...
  if (!ready) {
    const hint = serverLog.trim().length > 0 ? `
